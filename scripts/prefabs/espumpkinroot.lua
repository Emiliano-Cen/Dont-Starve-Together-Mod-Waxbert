local Assets = { 
    Asset("ANIM", "anim/espumpkinroot.zip"),
    Asset("ANIM", "anim/espumpkinroot_ground.zip"),
...
local function MainFunction()
    ...
    minimap:SetIcon("espumpkinroot.tex")
	
    inst.AnimState:SetBank("espumpkinroot_ground")
    inst.AnimState:SetBuild("espumpkinroot_ground")
    inst.AnimState:PlayAnimation("anim")

    inst:AddTag("espumpkinroot")
    inst:AddTag("waxxxer")
    ...
    inst.components.inventoryitem.imagename = "espumpkinroot"
    inst.components.inventoryitem.atlasname = "images/inventoryimages/espumpkinroot.xml"
    ...
    inst:DoPeriodicTask(0.1, function(inst)
        ...
        if owner and owner:HasTag("player") and owner.prefab ~= "waxbert" then
            if owner.components.inventory then
                ...
                local items = owner.components.inventory:FindItems(function(item) return item.prefab == "espumpkinroot" end)
...
return  Prefab("common/inventory/espumpkinroot", MainFunction, Assets)