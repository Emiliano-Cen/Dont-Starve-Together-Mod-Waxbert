local Assets = { 
    Asset("ANIM", "anim/esforestfruits.zip"),
    Asset("ANIM", "anim/esforestfruits_ground.zip"),
...
local function MainFunction()
    ...
    minimap:SetIcon("esforestfruits.tex")
	
    inst.AnimState:SetBank("esforestfruits_ground")
    inst.AnimState:SetBuild("esforestfruits_ground")
    inst.AnimState:PlayAnimation("anim")

    inst:AddTag("esforestfruits")
    inst:AddTag("waxxxer")
    ...
    inst.components.inventoryitem.imagename = "esforestfruits"
    inst.components.inventoryitem.atlasname = "images/inventoryimages/esforestfruits.xml"
    ...
    inst.components.edible:SetOnEatenFn(function(inst, eater)
        ...
        local function spawn_bees_or_butterflies(eater, spawn_count)
            ...
            eater:DoTaskInTime(spawn_period, function() spawn_bees_or_butterflies(eater, spawn_count + 1) end)
        end
    ...
    inst:DoPeriodicTask(0.1, function(inst)
        ...
        if owner and owner:HasTag("player") and owner.prefab ~= "waxbert" then
            if owner.components.inventory then
                ...
                local items = owner.components.inventory:FindItems(function(item) return item.prefab == "esforestfruits" end)
...
return  Prefab("common/inventory/esforestfruits", MainFunction, Assets)